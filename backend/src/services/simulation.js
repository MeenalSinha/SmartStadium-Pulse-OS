...
class SimulationEngine extends EventEmitter {
  ...
  async init() {
    try {
      ...
      log.info(
        { mode: this.mode, orders: this.orders.length },
        "Simulation state restored from DB",
      );
    } catch (err) {
      log.warn(
        { err: err.message },
        "Could not restore sim state from DB — using defaults",
      );
    }
  }
  ...
  getMetrics() {
    ...
    const congestionReduced = Math.max(
      0,
      Math.min(99, Math.round(((baseline - avg) / baseline) * 100 + 30)),
    );
    const waitTimeReduced = Math.max(
      0,
      Math.min(99, Math.round(((baseline - avg) / baseline) * 80 + 25)),
    );
