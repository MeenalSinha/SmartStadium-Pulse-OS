...
router.post("/route", (req, res) => {
  ...
  if (!result.path || result.path.length === 0) {
    return res
      .status(422)
      .json({ error: "No route found between these zones." });
  }
  ...
  const avgDensity =
    pathDetails.reduce((a, z) => a + z.density, 0) / pathDetails.length;
  const pointsEarned = avgDensity < 0.4 ? 15 : avgDensity < 0.6 ? 8 : 3;

  log.info(
    { from, to, hops: result.path.length, cost: result.cost },
    "Route calculated",
  );
...
router.get("/queue", shortCache, (_req, res) => {
  const stalls = STALLS.map((stall) => {
    ...
    return {
      ...
      level:
        zoneDensity > 0.7 ? "busy" : zoneDensity > 0.4 ? "moderate" : "quiet",
    };
  });
...
router.post(
  ...
  catchAsync(async (req, res) => {
    ...
    log.info(
      { orderId: order.id, stall: stall.name, userId: order.userId },
      "Order placed",
    );
    res.json(order);
  }),
...
router.get("/recommendations", (_req, res) => {
  ...
  const recs = [
    ...
    {
      type: "order",
      title: `Order from ${bestStall.name}`,
...
router.post(
  ...
  catchAsync(async (req, res) => {
    ...
    if (!["warning", "critical", "info"].includes(type)) {
      return res
        .status(400)
        .json({ error: "Type must be warning, critical, or info." });
    }
    ...
    [alert, ...sim.alerts].forEach((a) => {
      if (
        !alertMap.has(a.zone) ||
        a.timestamp > alertMap.get(a.zone).timestamp
      ) {
...
router.post("/best-route", (req, res) => {
  ...
  for (const [to] of candidates) {
    ...
    if (result.path && result.path.length > 0 && result.cost !== Infinity) {
      ...
      log.info(
        { from, to, improvement: sim.density[from] - destDensity },
...
router.get("/crowd-status", (_req, res) => {
  ...
  const critical = densities
    .filter(([, d]) => d > 0.75)
    .map(([id]) => ({ id, name: ZONES[id].name, density: sim.density[id] }));
  const quiet = densities
    .filter(([, d]) => d < 0.35)
    .map(([id]) => ({ id, name: ZONES[id].name, density: sim.density[id] }));
  const bestStall = STALLS.map((s) => ({
    ...s,
  ...
  const statusMessages = {
    critical:
      "Stadium is very crowded. Follow AI guidance to avoid congestion.",
