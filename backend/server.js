...
io.on('connection', (socket) => {
  ...
  log.info({ socketId: socket.id, total: sim.connectedClients }, 'Client connected');
  socket.emit('simulation_update', sim.snapshot());

  socket.on('disconnect', () => {
    sim.connectedClients = Math.max(0, sim.connectedClients - 1);
    log.info({ socketId: socket.id, total: sim.connectedClients }, 'Client disconnected');
  });
});
...
sim.on('proactiveNudge', (nudge) => {
  ...
  log.info({ fromZone: nudge.fromZone, toZone: nudge.toZone }, 'Proactive nudge broadcast');
});
...
async function boot() {
  await sim.init();
  server.listen(PORT, () => {
    log.info({ port: PORT, origins: ALLOWED_ORIGINS }, 'SmartStadium Pulse OS ready');
  });
}
