...
describe('isValidZone()', () => {
  ...
  test('rejects SQL injection',   () => expect(isValidZone("A'; DROP TABLE zones--")).toBe(false));
  test('rejects object input',    () => expect(isValidZone({ id: 'A' })).toBe(false));
});
...
describe('sanitizeUserId()', () => {
  ...
  test('returns guest for number', () => expect(sanitizeUserId(42)).toBe('guest'));
...
describe('requireAdminKey()', () => {
  const mockReq = (headers = {}) => ({
    headers, ip: '127.0.0.1', path: '/api/simulate',
  });
  const mockRes = () => {
  ...
  test('rejects key with wrong length (timing-safe check)', () => {
    ...
    const res  = mockRes();
    const next = jest.fn();
    requireAdminKey(mockReq({ 'x-admin-key': 'short-but-different' }), res, next);
    expect(next).not.toHaveBeenCalled();
  });
});
...
describe('catchAsync()', () => {
  ...
  test('calls next(err) when async handler throws', async () => {
    ...
    const fn   = catchAsync(async () => { throw err; });
    const next = jest.fn();
    await fn({}, {}, next);
  ...
  test('does NOT call next when handler resolves successfully', async () => {
    ...
    const fn   = catchAsync(async (_req, res) => { res.json({ ok: true }); });
    const next = jest.fn();
    await fn({}, res, next);
    expect(next).not.toHaveBeenCalled();
