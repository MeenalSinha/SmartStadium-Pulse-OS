...
describe('POST /api/route', () => {
  test('A→G returns valid path', async () => {
    const res = await request(app).post('/api/route').send({ from: 'A', to: 'G' });
    expect(res.status).toBe(200);
  ...
  test('B→B returns zero-cost single-stop path', async () => {
    const res = await request(app).post('/api/route').send({ from: 'B', to: 'B' });
    expect(res.status).toBe(200);
  ...
  test('path nodes have all required fields', async () => {
    const res = await request(app).post('/api/route').send({ from: 'A', to: 'H' });
  ...
  test('invalid zone → 400', async () =>
    expect((await request(app).post('/api/route').send({ from: 'Z', to: 'A' })).status).toBe(400));
  test('SQL injection → 400', async () =>
    expect((await request(app).post('/api/route').send({ from: "A'; DROP TABLE--", to: 'G' })).status).toBe(400));
  test('numeric zone → 400', async () =>
    expect((await request(app).post('/api/route').send({ from: 1, to: 'G' })).status).toBe(400));
...
describe('POST /api/order', () => {
  ...
  test('XSS in items is stripped', async () => {
    const res = await request(app).post('/api/order')
      .send({ stallId: 's1', items: ['<script>alert(1)</script>'], userId: 'u1' });
    expect(res.status).toBe(200);
  ...
  test('ordersStored increments after order', async () => {
    ...
    await request(app).post('/api/order').send({ stallId: 's2', items: ['1x Pizza'], userId: 'u1' });
    const after = (await request(app).get('/api/health')).body.ordersStored;
    expect(after).toBe(before + 1);
  });

  test('invalid stall → 400', async () =>
    expect((await request(app).post('/api/order').send({ stallId: 'bad', items: ['x'], userId: 'u1' })).status).toBe(400));
  test('empty items → 400', async () =>
    expect((await request(app).post('/api/order').send({ stallId: 's1', items: [], userId: 'u1' })).status).toBe(400));
  test('non-array items → 400', async () =>
    expect((await request(app).post('/api/order').send({ stallId: 's1', items: 'burger', userId: 'u1' })).status).toBe(400));
  test('>20 items → 400', async () =>
    expect((await request(app).post('/api/order').send({ stallId: 's1', items: Array(21).fill('x'), userId: 'u1' })).status).toBe(400));
});
...
describe('POST /api/simulate', () => {
  test('all 4 valid modes accepted', async () => {
    for (const mode of ['normal','pre_match','halftime','exit_rush']) {
      const res = await request(app).post('/api/simulate').set('x-admin-key', 'test-secret').send({ mode });
      expect(res.status).toBe(200);
  ...
  test('invalid mode → 400', async () =>
    expect((await request(app).post('/api/simulate').set('x-admin-key', 'test-secret').send({ mode: 'invalid' })).status).toBe(400));
  test('__proto__ injection → 400', async () =>
    expect((await request(app).post('/api/simulate').set('x-admin-key', 'test-secret').send({ mode: '__proto__' })).status).toBe(400));
...
describe('Edge cases', () => {
  test('oversized body returns 413 or 400', async () => {
    ...
    const res = await request(app).post('/api/order')
      ...
      .send(JSON.stringify({ stallId:'s1', items:[bigString], userId:'u1' }));
  ...
  test('multiple orders are all retrievable via health endpoint', async () => {
    ...
    for (const stallId of ['s1','s2','s3']) {
      await request(app).post('/api/order')
        .send({ stallId, items: [`1x item from ${stallId}`], userId: 'db_test' });
    }
    const after = (await request(app).get('/api/health')).body.ordersStored;
...
describe('Async route error handling', () => {
  test('DB failure in /api/order returns 500 not a hang', async () => {
    ...
    simModule.addOrder = async () => { throw new Error('DB write failed'); };
...
describe('POST /api/alert', () => {
  test('valid alert injected and returned', async () => {
    const res = await request(app).post('/api/alert')
      .send({ zone: 'B', message: 'Crowd control needed at West Stand', type: 'warning' });
    expect(res.status).toBe(200);
  ...
  test('critical type accepted', async () => {
    const res = await request(app).post('/api/alert')
      .send({ zone: 'A', message: 'Emergency at North Gate', type: 'critical' });
    expect(res.status).toBe(200);
  ...
  test('alert appears in subsequent /api/alerts response', async () => {
    await request(app).post('/api/alert')
      .send({ zone: 'D', message: 'East Stand — stewards deployed', type: 'warning' });
    const alertsRes = await request(app).get('/api/alerts');
    const found = alertsRes.body.alerts.some(a => a.zone === 'D' && a.source === 'staff');
    expect(found).toBe(true);
  });

  test('invalid zone returns 400', async () =>
    expect((await request(app).post('/api/alert').send({ zone: 'Z', message: 'test' })).status).toBe(400));
  test('missing message returns 400', async () =>
    expect((await request(app).post('/api/alert').send({ zone: 'A', message: '' })).status).toBe(400));
  test('invalid type returns 400', async () =>
    expect((await request(app).post('/api/alert').send({ zone: 'A', message: 'test', type: 'fake' })).status).toBe(400));
  test('message capped at 200 chars', async () => {
    const long = 'x'.repeat(300);
    const res = await request(app).post('/api/alert').send({ zone: 'E', message: long });
    expect(res.status).toBe(200);
