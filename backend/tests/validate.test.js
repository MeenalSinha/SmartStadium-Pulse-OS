...
describe("isValidZone()", () => {
  ...
  test("is case-sensitive (lowercase rejected)", () =>
    expect(isValidZone("a")).toBe(false));
});
...
describe("isValidMode()", () => {
  ...
  test("rejects unknown mode", () =>
    expect(isValidMode("full_capacity")).toBe(false));
...
describe("sanitizeItems()", () => {
  test("returns cleaned array for valid input", () => {
    expect(sanitizeItems(["1x Burger", "2x Fries"])).toEqual([
      "1x Burger",
      "2x Fries",
    ]);
  });
  ...
  test("returns null for non-array", () =>
    expect(sanitizeItems("burger")).toBeNull());
