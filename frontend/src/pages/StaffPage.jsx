...
const INITIAL_STAFF = [
  { id: 'st1', name: 'Team Alpha', role: 'Crowd Control', count: 4, status: 'available', zone: null },
  { id: 'st2', name: 'Team Beta',  role: 'Stewards',      count: 6, status: 'deployed',  zone: 'B' },
  { id: 'st3', name: 'Team Gamma', role: 'Medical',       count: 2, status: 'standby',   zone: null },
  { id: 'st4', name: 'Team Delta', role: 'Security',      count: 5, status: 'deployed',  zone: 'D' },
  { id: 'st5', name: 'Team Echo',  role: 'Crowd Control', count: 3, status: 'available', zone: null },
];
...
const ALERT_MESSAGES = {
  critical: (zone) => `CRITICAL: Immediate crowd control required at ${ZONE_NAMES[zone]} — unsafe density`,
  moderate: (zone) => `CAUTION: Stewards needed at ${ZONE_NAMES[zone]} to guide crowd flow`,
};
...
export default function StaffPage({ simData }) {
  ...
  const sendAlert = useCallback(async (zoneId, urgency) => {
    ...
    try {
      ...
      await api.sendStaffAlert(zoneId, message, urgency === 'critical' ? 'critical' : 'warning');
      ...
      setTimeout(() => setAlertStatus(prev => {
  ...
  const statusBadge = { available: 'badge-green', deployed: 'badge-blue', standby: 'badge-yellow' };

  return (
    <div>
      <div className="section-title">Staff Dispatch</div>
      <div className="section-desc">AI-recommended deployment based on real-time crowd data</div>

      <div className="grid-3 mb-4">
        <div className="metric-card">
          <div className="metric-label">Teams Available</div>
          <div className="metric-value">{staff.filter(s => s.status === 'available').length}</div>
          <div className="metric-change">ready for deployment</div>
        </div>
        <div className="metric-card">
          <div className="metric-label">Teams Deployed</div>
          <div className="metric-value">{staff.filter(s => s.status === 'deployed').length}</div>
          <div className="metric-change">active in field</div>
        </div>
        <div className="metric-card">
          <div className="metric-label">Zones Needing Cover</div>
          <div className="metric-value" style={{ color: highZones.length > 0 ? 'var(--red)' : 'var(--green)' }}>
            {highZones.length}
          </div>
          <div className={`metric-change${highZones.length > 0 ? ' negative' : ''}`}>
          ...
          {highZones.length === 0 ? (
            <div className="empty-state">
              <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
                <circle cx="9" cy="7" r="4"/><path d="M3 21a6 6 0 0112 0"/>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {highZones.map(zone => {
                const availableTeam = staff.find(s => s.status === 'available');
                ...
                return (
                  <div key={zone.id} style={{
                    ...
                    background: zone.level === 'critical' ? 'var(--red-light)' : 'var(--yellow-light)',
                  }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                      <div>
                        <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-primary)' }}>{zone.name}</span>
                        <span style={{ fontSize: 11, color: 'var(--text-muted)', marginLeft: 8 }}>Zone {zone.id}</span>
                      </div>
                      <span className={`badge badge-${zone.level === 'critical' ? 'red' : 'yellow'}`}>
                    ...
                    <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                      {availableTeam ? (
                        ...
                        <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>No teams available</span>
                      )}
              ...
              <div key={s.id} style={{
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>
                    {s.role}{s.zone ? ` — Zone ${s.zone} (${ZONE_NAMES[s.zone]})` : ''}
                  </div>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                  <span className={`badge ${statusBadge[s.status]}`} style={{ textTransform: 'capitalize' }}>
                    {s.status}
                  </span>
                  {s.status === 'deployed' && (
                    <button className="btn btn-secondary btn-sm" onClick={() => recall(s.id)}>Recall</button>
                  )}
                </div>
              </div>
            ))}
