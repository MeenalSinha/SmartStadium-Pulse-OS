...
export default function HeatmapPage({ simData }) {
  ...
  useEffect(() => {
    if (simData?.density) {
      setZones(Object.entries(simData.density).map(([id, d]) => {
    ...
    <div>
      ...
      {error && (
        <div style={{ padding: '10px 14px', background: 'var(--red-light)', border: '1px solid #FCA5A5',
          borderRadius: 'var(--radius-md)', fontSize: 12, color: 'var(--red)', marginBottom: 16 }}>
          {error}
        </div>
      )}
            ...
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              {['low','moderate','high','critical'].map(l => (
                <div key={l} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                  ...
                  <span style={{ fontSize: 10, color: 'var(--text-muted)', textTransform: 'capitalize' }}>{l}</span>
                </div>
              ))}
            </div>
          </div>
          <StadiumMap density={density} selectedZone={selected} onZoneClick={setSelected} />
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          {selectedZone && (
            <div className="card" style={{ borderColor: 'var(--accent)', background: 'var(--accent-light)' }}>
              <div className="card-title" style={{ marginBottom: 10 }}>{selectedZone.name}</div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                {[
                  { label: 'Occupancy',     value: `${pct(selectedZone.density)}%` },
                  { label: 'Current Count', value: selectedZone.currentCount.toLocaleString() },
                  ...
                  <div key={label}>
                    ...
                    <div style={{ fontSize: 16, fontWeight: 700, color: 'var(--accent)', marginTop: 2 }}>{value}</div>
                  </div>
                ))}
              </div>
            </div>
              ...
              {zones.map(z => (
                <div
                  ...
                  style={{
                    ...
                    background: selected === z.id ? 'var(--accent-light)' : 'var(--bg-card)',
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6, alignItems: 'center' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      ...
                      <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-primary)' }}>{z.name}</span>
                    </div>
                    <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-primary)' }}>{pct(z.density)}%</span>
                  </div>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4 }}>
                    <span style={{ fontSize: 10, color: 'var(--text-muted)' }}>
                      {z.currentCount.toLocaleString()} / {z.capacity.toLocaleString()}
                    </span>
