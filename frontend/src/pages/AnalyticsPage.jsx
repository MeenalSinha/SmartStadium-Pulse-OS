...
export default function AnalyticsPage({ simData }) {
  ...
  return (
    <div>
      ...
      <div className="section-desc">Live performance metrics and AI impact analysis</div>

      <div className="grid-4 mb-4">
        {[
          { label: 'Avg Wait Reduction', value: `${metrics.waitTimeReduced ?? '—'}%`, sub: 'vs unoptimized', pos: true },
          { label: 'Routing Improvement', value: `${metrics.routingImprovement ?? 63}%`, sub: 'faster paths', pos: true },
          { label: 'Orders Processed', value: (metrics.ordersProcessed ?? 0).toLocaleString(), sub: 'this session via ZeroQueue', pos: true },
          { label: 'Avg Density', value: `${Math.round((metrics.avgDensity ?? 0.35) * 100)}%`, sub: 'stadium-wide', pos: null },
        ].map(m => (
          <div key={m.label} className="metric-card">
            <div className="metric-label">{m.label}</div>
            <div className="metric-value">{m.value}</div>
            <div className={`metric-change${m.pos === false ? ' negative' : ''}`}>{m.sub}</div>
          </div>
        ))}
      </div>
      ...
      <div className="grid-2 mb-4">
        <div className="card">
          <div className="card-title" style={{ marginBottom: 16 }}>
            ...
            <span style={{ fontSize: 11, color: 'var(--text-muted)', marginLeft: 8, fontWeight: 400 }}>live</span>
          </div>
          {history.length < 2 ? (
            <div className="empty-state" style={{ height: 220 }}><p>Collecting live data…</p></div>
          ) : (
            <ResponsiveContainer width="100%" height={220}>
              <AreaChart data={history}>
                ...
                <XAxis dataKey="hour" tick={{ fontSize: 9, fill: '#9CA3AF' }} interval="preserveStartEnd" />
                <YAxis tick={{ fontSize: 10, fill: '#9CA3AF' }} domain={[0, 100]} />
                <Tooltip contentStyle={{ fontSize: 12, borderRadius: 8, border: '1px solid var(--border)' }} />
                <Legend wrapperStyle={{ fontSize: 11 }} />
                <Area type="monotone" dataKey="optimized" stroke="#0E9F6E" fill="url(#opt)" strokeWidth={2} name="AI Optimized %" />
                <Area type="monotone" dataKey="baseline" stroke="#E02424" fill="url(#base)" strokeWidth={2} name="Unoptimized Baseline %" />
              </AreaChart>
            </ResponsiveContainer>
          )}
        </div>
        ...
        <div className="card">
          <div className="card-title" style={{ marginBottom: 16 }}>Orders & Density Trend</div>
          {history.length < 2 ? (
            <div className="empty-state" style={{ height: 220 }}><p>Collecting live data…</p></div>
          ) : (
            <ResponsiveContainer width="100%" height={220}>
              <LineChart data={history}>
                ...
                <Legend wrapperStyle={{ fontSize: 11 }} />
                <Line yAxisId="left" type="monotone" dataKey="orders" stroke="#1A56DB" strokeWidth={2} dot={false} name="Orders" />
                <Line yAxisId="right" type="monotone" dataKey="density" stroke="#D03801" strokeWidth={2} dot={false} name="Density %" />
              </LineChart>
            </ResponsiveContainer>
          )}
        </div>
      ...
      <div className="grid-2">
        <div className="card">
          <div className="card-title" style={{ marginBottom: 16 }}>System Performance Score</div>
          <ResponsiveContainer width="100%" height={240}>
            <RadarChart data={radarData}>
              <PolarGrid stroke="#E4E7EC" />
              <PolarAngleAxis dataKey="subject" tick={{ fontSize: 11, fill: '#6B7280' }} />
              <PolarRadiusAxis angle={90} domain={[0, 100]} tick={{ fontSize: 9 }} />
              <Radar name="Score" dataKey="score" stroke="#1A56DB" fill="#1A56DB" fillOpacity={0.12} strokeWidth={2} />
              <Tooltip contentStyle={{ fontSize: 12, borderRadius: 8 }} />
            </RadarChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="card">
          <div className="card-title" style={{ marginBottom: 14 }}>Key Performance Indicators</div>
          <table className="data-table">
            ...
            <tbody>
              {radarData.map(d => (
                <tr key={d.subject}>
                  <td style={{ fontWeight: 500, color: 'var(--text-primary)' }}>{d.subject}</td>
                  <td style={{ fontWeight: 700 }}>{d.score}/100</td>
                  <td>
                    <span className={`badge badge-${d.score >= 90 ? 'green' : d.score >= 75 ? 'blue' : 'yellow'}`}>
                      {d.score >= 90 ? 'Excellent' : d.score >= 75 ? 'Good' : 'Fair'}
                    </span>
                  </td>
                </tr>
              ))}
