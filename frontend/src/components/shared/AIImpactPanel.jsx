...
export default function AIImpactPanel({ metrics, mode, density }) {
  ...
  const MODE_BASELINE = {
    normal: 0.25, pre_match: 0.65, halftime: 0.85, exit_rush: 0.90,
  };
  ...
  return (
    <div className={`ai-impact-panel ${flashClass}`} style={{ marginBottom: 20 }}>
      <div className="scan-line" />

      {/* Header */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
        <div>
          <div className="ai-impact-title">⚡ Pulse OS — Behavioral AI Engine</div>
          <div className="ai-impact-headline">
            Self-optimizing crowd<br />redistribution — live
          </div>
          <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 8, maxWidth: 340 }}>
            Actively routing {metrics?.activeUsers?.toLocaleString() ?? '850+'} fans away from congestion
            using real-time Dijkstra pathfinding + density-weighted nudging
          </div>
        </div>

        {/* Big animated counter */}
        <div style={{ textAlign: 'right', flexShrink: 0 }}>
          <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--green)', fontFamily: "'JetBrains Mono', monospace", marginBottom: 4 }}>
            CONGESTION REDUCED
          </div>
      ...
      <div className="before-after-container" style={{ marginBottom: 20 }}>
        <div className="before-block">
          ...
          <div style={{ fontSize: 10, color: 'var(--text-muted)', marginTop: 4 }}>avg zone density</div>
        </div>

        <div className="arrow-block">
          <svg width="32" height="20" viewBox="0 0 32 20" fill="none">
            <path d="M2 10h28M20 2l10 8-10 8" stroke="url(#arrowGrad)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
            <defs>
          ...
          <div style={{ fontSize: 9, color: 'var(--text-muted)', fontFamily: "'JetBrains Mono', monospace", letterSpacing: '0.05em' }}>AI ACTIVE</div>
        </div>

        <div className="after-block">
          <div className="after-label">With Pulse OS</div>
          <div className="after-value">{currentPct}%</div>
          <div style={{ fontSize: 10, color: 'var(--text-muted)', marginTop: 4 }}>current avg density</div>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
        {[
          ...
          <div key={stat.label} style={{
            ...
            <div style={{ fontSize: 9, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--text-muted)', fontFamily: "'JetBrains Mono', monospace", marginBottom: 6 }}>
              {stat.label}
            </div>
            <div style={{ fontSize: 24, fontWeight: 700, color: stat.color, fontFamily: "'Bricolage Grotesque', sans-serif", letterSpacing: '-0.04em', lineHeight: 1 }}>
              {stat.value}
            </div>
            <div style={{ fontSize: 10, color: 'var(--text-muted)', marginTop: 4 }}>{stat.sub}</div>
          </div>
        ))}
      </div>
      ...
      {/* Status bar */}
      {!isActive && (
        <div style={{ marginTop: 14, padding: '8px 14px', background: 'rgba(245,158,11,0.1)', border: '1px solid rgba(245,158,11,0.2)', borderRadius: 'var(--radius-sm)', fontSize: 11, color: 'var(--yellow)', display: 'flex', alignItems: 'center', gap: 8 }}>
          <div className="live-dot" style={{ background: 'var(--yellow)' }} />
          Trigger a crowd scenario above to activate behavioral AI redistribution
        </div>
      )}
    </div>
  );
