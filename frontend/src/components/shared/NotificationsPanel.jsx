      ...
      {toasts.map(toast => (
        <div key={toast.key} className={`toast toast-${toast.type}`}>
          <div style={{
            ...
            background: toast.type === 'critical' ? 'var(--red-light)' : 'var(--yellow-light)',
            display: 'flex', alignItems: 'center', justifyContent: 'center',
          }}>
            {toast.type === 'critical' ? (
              <svg width="13" height="13" viewBox="0 0 16 16" fill="none" stroke="var(--red)" strokeWidth="2">
                <path d="M8 1l7 14H1L8 1z"/><path d="M8 7v4"/><circle cx="8" cy="13" r="0.5" fill="var(--red)" />
              </svg>
            ) : (
              <svg width="13" height="13" viewBox="0 0 16 16" fill="none" stroke="var(--yellow)" strokeWidth="2">
                <circle cx="8" cy="8" r="7"/><path d="M8 5v4"/><circle cx="8" cy="12" r="0.5" fill="var(--yellow)" />
              </svg>
            )}
          </div>
