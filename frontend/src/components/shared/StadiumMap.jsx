...
import { getDensityColor, getDensityBg, pct, clampDensity } from '../../utils/helpers';
...
const getDarkBg = (density) => {
  ...
  if (density > 0.50) return 'rgba(249,115,22,0.12)';
  if (density > 0.25) return 'rgba(245,158,11,0.12)';
  return 'rgba(16,185,129,0.12)';
};

export default function StadiumMap({ density = {}, path = [], selectedZone, onZoneClick, compact = false }) {
  ...
  return (
    <svg viewBox="0 0 400 420" width={W} height={H} style={{ display:'block', margin:'0 auto' }}>
      <defs>
        <filter id="glow-red">
      ...
      {/* Pitch */}
      <ellipse cx="200" cy="210" rx="96" ry="88"
        fill="rgba(16,185,129,0.06)" stroke="rgba(16,185,129,0.15)" strokeWidth="1.5"/>
      <ellipse cx="200" cy="210" rx="56" ry="50"
        fill="none" stroke="rgba(16,185,129,0.10)" strokeWidth="1" strokeDasharray="4,3"/>
      <line x1="200" y1="122" x2="200" y2="298" stroke="rgba(16,185,129,0.10)" strokeWidth="1"/>
      <text x="200" y="214" textAnchor="middle" fontSize="8" fill="rgba(16,185,129,0.35)"
        fontFamily="'JetBrains Mono',monospace" fontWeight="700" letterSpacing="0.1em">PITCH</text>
      ...
      {path.length > 1 && path.slice(0, -1).map((id, i) => {
        const from = ZONES[id], to = ZONES[path[i+1]];
        if (!from || !to) return null;
        return (
          <circle key={`mid-${id}-${path[i+1]}`}
            cx={(from.x+to.x)/2} cy={(from.y+to.y)/2} r={4}
            fill="#3B82F6" opacity={0.8} filter="url(#glow-blue)"/>
        );
      })}
      ...
      {Object.values(ZONES).map(zone => {
        ...
        const glowFilter = isCritical ? 'url(#glow-red)' : inPath ? 'url(#glow-blue)' : d < 0.25 ? 'url(#glow-green)' : undefined;
          ...
          >
            ...
            {isCritical && (
              <circle cx={zone.x} cy={zone.y} r={zone.r + 10}
                ...
                <animate attributeName="r" values={`${zone.r+6};${zone.r+14};${zone.r+6}`} dur="1.8s" repeatCount="indefinite"/>
            ...
            {/* Zone ID */}
            <text x={zone.x} y={zone.y-2} textAnchor="middle"
            ...
            {/* Density % */}
            <text x={zone.x} y={zone.y+10} textAnchor="middle"
      ...
      {Object.values(ZONES).map(zone => {
        ...
        const off = offsets[zone.id] || { dx:0, dy:-zone.r-6, anchor:'middle' };
        return (
          <text key={`label-${zone.id}`}
            x={zone.x+off.dx} y={zone.y+off.dy}
            textAnchor={off.anchor} fontSize="7.5"
