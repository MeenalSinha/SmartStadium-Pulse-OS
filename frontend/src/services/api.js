...
async function apiFetch(path, options = {}) {
  ...
  try {
    ...
    throw new Error(`Network error: cannot reach backend at ${BASE}. Is it running?`);
  }
  if (!res.ok) {
...
export const api = {
  ...
  getBestRoute: (from) => apiFetch('/api/best-route', {
    method: 'POST',
    body: JSON.stringify({ from }),
  }),

  placeOrder: (stallId, items, userId) => apiFetch('/api/order', {
    method: 'POST',
    body: JSON.stringify({ stallId, items, userId }),
  }),

  sendStaffAlert: (zone, message, type = 'warning') => apiFetch('/api/alert', {
    method: 'POST',
    body: JSON.stringify({ zone, message, type }),
  }),

  setSimMode: (mode) => apiFetch('/api/simulate', {
    method: 'POST',
